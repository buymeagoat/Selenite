...
- **Workspace role guard**: Before any file edit, run `scripts/workspace-guard.ps1` and confirm `.workspace-role` is `dev`. If the role is `prod`, stop and wait for explicit approval plus `SELENITE_ALLOW_PROD_WRITES=1`. If any tool output references the prod path while working on dev tasks, stop and correct the working directory before proceeding.
