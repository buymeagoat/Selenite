...
|-----|--------|----------|-------------|
| Frontend actions use placeholders (upload, restart, delete, tag CRUD, settings save) | User cannot persist real changes | P0 | **Status: [OK] Resolved (Nov 21, 2025)** - Dashboard + modal actions now call live services with toast/error handling. |
| Missing transcript download formats integration (UI triggers without backend verification) | Export may fail silently | P1 | **Status: [OK] Resolved (Nov 15, 2025)** - Export API wired to download buttons; failures surface via toasts. |
| No real-time job status updates from backend | UI polling simulates progress but doesn't reflect actual transcription state | P0 | **Status: [OK] Resolved (Nov 18, 2025)** - Dashboard polling hits `/jobs` and displays queue state instead of mock progress. |
| Settings persistence not implemented | User preferences reset on page reload | P1 | **Status: [OK] Resolved (Nov 18, 2025)** - Settings page uses GET/PUT `/settings` with success/error feedback. |
| Tag CRUD operations stubbed | Cannot create/edit/delete tags from UI | P1 | **Status: [OK] Resolved (Nov 18, 2025)** - Tag management wired to POST/PATCH/DELETE endpoints. |
| Job restart creates new placeholder job (not real restart) | Restart button doesn't work | P2 | **Status: [OK] Resolved (Nov 17, 2025)** - Restart button calls `/jobs/{id}/restart`; verified via E2E. |
...
| Gap | Impact | Priority | Remediation |
|-----|--------|----------|-------------|
| Frontend tests not executed (runner stability issue) | Unknown regression risk | P0 | **Status: [OK] Resolved (Nov 20, 2025)** - Vitest coverage runs via `run-tests.ps1` + CI. |
| Missing integration/E2E tests (Increment 19 skipped) | Critical workflow unverified | P0 | **Status: [OK] Resolved (Nov 21, 2025)** - `npm run e2e:full` executes 85 Playwright specs across browsers. |
| No coverage reporting enforced | Hard to measure quality gates compliance | P1 | **Status: [OK] Resolved (Nov 21, 2025)** - Backend `pytest --cov`, frontend `vitest --coverage`, and CI gating configured. |
...
| Gap | Impact | Priority | Remediation |
|-----|--------|----------|-------------|
| No rate limiting / brute force protection | Credential stuffing risk | P0 | **Status: [OK] Resolved (Nov 10, 2025)** - Rate limiting middleware active on auth APIs. |
| No password complexity / rotation policy | Weak credential hygiene | P1 | **Status: [OK] Resolved (Nov 12, 2025)** - Validation enforces length/entropy; docs call for periodic rotation. |
| No account lockout on repeated failures | Susceptible to brute force | P1 | **Status: [OK] Resolved (Nov 12, 2025)** - Failed attempt counter introduces temporary lockouts. |
| No JWT refresh / short-lived access tokens | Risk if token leaked | P2 | Implement refresh token pair, shorter access token TTL |
| Logging not structured / no audit trail | Hard incident investigation | P2 | **Status: [OK] Resolved (Nov 15, 2025)** - Structured logging/rotation configured; audit info captured. |
...
| No centralized logs or rotation policy | Disk bloat & lost context | P1 | **Status: [OK] Resolved (Nov 15, 2025)** - File logging disabled in local builds; rotation + structured logging documented. |
...
| No backup automation | Data loss risk | P1 | Manual backup/restore verification scripts added; scheduled backups + offsite storage still needed. |
...
| **DATA-002** | Data | Add automated backup strategy | P1 | 3 | RC7 | Pending | Senior Dev | Manual backup/restore verification scripts added; scheduled backup + offsite storage still required. |
...
- [OK] **TEST-001 through TEST-004**: Status = Done
- [OK] **FUNC-001 through FUNC-003**: Status = Done
- [OK] **CI-001**: Status = Done
- [OK] **Backend test suite**: 100% pass rate (129 tests green)
- [OK] **Frontend test suite**: Executes to completion without hangs; >=95% pass rate
- [OK] **CI pipeline**: Green build on main branch
- [OK] **Vertical slice validated**: Manual test completes Login -> Upload file -> Wait for processing -> View job detail -> Download transcript (all steps succeed)
- [OK] **Integration debt visible**: All remaining placeholder stubs tagged with `// TODO(API):` markers
