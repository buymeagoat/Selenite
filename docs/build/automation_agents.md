...
- Quality: bundle-size budget, accessibility smoke (axe), perf smoke on critical endpoints, optional i18n missing-strings check.
...
- CI (separate workflows):
  ...
  - `security.yml` runs secrets + audit + license + static security lint.
  - `tests.yml` runs test suites; retries flakies once.
  - `quality.yml` runs bundle/accessibility/perf smoke.
